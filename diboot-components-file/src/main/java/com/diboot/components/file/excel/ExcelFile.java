...
/**
 * Excel文件对象，用于生成
 * @version since v1.0.1
 * @author Mazc@dibo.ltd
 * @version 2018/12/3
 */
public class ExcelFile implements Serializable {
    private static final long serialVersionUID = 799616235579102900L;
     ...
     * 添加新的Sheet
     * @param sheetName
     * @param dataRows: List<LinkedHashMap> 或者 List<String[]>
     */
    public void addSheet(String sheetName, List dataRows){
        Object obj = dataRows.get(0);
        if(obj instanceof LinkedHashMap){
            List<LinkedHashMap> newDataRows = (List<LinkedHashMap>) dataRows;
            SheetWrapper sheet = new SheetWrapper(newDataRows);
            sheet.setSheetName(sheetName);
            sheetList.add(sheet);
        }
        else if(obj.getClass().isArray()){
            List<String> headers = Arrays.asList((String[])obj);
            List<String[]> dataRowsFrom1 = new ArrayList<>();
            if(dataRows.size() > 1){
                for(int i=1; i<dataRows.size(); i++){
                    dataRowsFrom1.add((String[])dataRows.get(i));
                }
            }
            addSheet(sheetName, headers, dataRowsFrom1);
        }
    }
    ...
    private boolean generateXlsFile(String fullPath){
        ...
        try {
            ...
            if(fullPath.endsWith(".xls")){
                wb = generateXls(sheetList);
            }
            else if(fullPath.endsWith(".xlsx")){
                wb = generateXlsx(sheetList);
            }
            else{
                logger.warn("无法识别的excel后缀: "+ S.substringAfterLast(fullPath, "\\."));
            }
            //创建文件流
    ...
    private static HSSFWorkbook generateXls(List<SheetWrapper> sheets) throws Exception{
        ...
        for(SheetWrapper wrapper : sheets){
            ...
            // 设置样式
            HSSFCellStyle headerCellStyle = (HSSFCellStyle) buildHeaderStyle(wb); // 样式对象
     ...
     * @throws Exception
     */
    private static XSSFWorkbook generateXlsx(List<SheetWrapper> sheets) throws Exception{
        if(sheets == null){
            return null;
        }
        ...
        for(SheetWrapper wrapper : sheets){
            ...
            // 设置样式
            XSSFCellStyle headerCellStyle = (XSSFCellStyle) buildHeaderStyle(wb); // 样式对象
    ...
    private static CellStyle buildHeaderStyle(Workbook wb){
        CellStyle headerCellStyle = wb.createCellStyle(); // 样式对象
        // 设置单元格的背景颜色为淡蓝色
        headerCellStyle.setFillForegroundColor(HSSFColor.HSSFColorPredefined.BLUE_GREY.getIndex());
